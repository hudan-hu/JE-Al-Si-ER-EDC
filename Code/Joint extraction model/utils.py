...
def generator(data, m,config,train=False):
    ...
    dropout_att = m['dropout_att']

    dropout_fcl_ner_keep = m['dropout_fcl_ner']
    dropout_fcl_rel_keep = m['dropout_fcl_rel']
    context_token_ids=m['context_token_ids']
    # batcher=m['batcher']
    # elmo_context_input=m['elmo_context_input']
    # elmo_embeddings=m['elmo_embeddings']
    ...
    if config.use_dropout == True and train==True:
        ...
        dropout_att_prob = config.dropout_att
    ...
    for bIdx in range(len(batches_embeddingIds)):  # 131
        ...
        # print("batch_embeddings",batch_embeddingIds,"shape",batch_embeddingIds.shape)
        ...
        yield {dropout_embedding_keep:dropout_embedding_prob,dropout_lstm_keep:dropout_lstm_prob,dropout_lstm_output_keep:dropout_lstm_output_prob,dropout_att:dropout_att_prob,
               ...
               tokens:batch_token,BIO: batch_bio,tokenIds:batch_tokenId,scoringMatrixGold:batch_scoringMatrix, seqlen:docs_length,
               doc_ids:batch_doc_id,context_token_ids:context}
