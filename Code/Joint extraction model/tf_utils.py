...
class model:
    ...
    def evaluate(self,eval_data,operations,set):
        ...
        if self.config.evaluation_method == "relaxed":
            ...
            if set == 'test':
                NER_socre_F1, REL_socre_F1 = evaluator.printInfo()
                return NER_socre_F1, REL_socre_F1, evaluator.getChunkedOverallAvgF1()
            else:
                evaluator.printInfo()
    ...
    def computeLoss(self,input_rnn, dropout_embedding_keep,dropout_lstm_keep,dropout_lstm_output_keep,dropout_att,
    ...
    def run(self):
        ...
        dropout_att = tf.placeholder(tf.float32, name="dropout_att")
        ...
        context_token_ids=tf.placeholder(tf.int32,shape=(None,None),name='context_token_ids')

        vocab_file = "vocab.txt"
        datadir = os.path.join('bilm', 'model')
        ...
        lossNER, lossREL, predicted_entity_tags_ids, predictedRel, rel_scores = self.computeLoss(input_rnn,
                                                                                ...
                                                                                dropout_lstm_keep,
                                                                                dropout_lstm_output_keep, dropout_att,seqlen,
